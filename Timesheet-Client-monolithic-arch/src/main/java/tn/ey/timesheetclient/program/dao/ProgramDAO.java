...
public interface ProgramDAO extends JpaRepository<Program,Long> {
    ...
    List<Program> findByChefprogramIdAndStatusIn(Long chefprogramId,List<Status> statuses);

    @Query("SELECT p.idprog AS idprog, p.numcontrat AS numcontrat, p.name AS name, p.status AS status, p.launchedat AS launchedat, p.image AS image " +
            "FROM Program p " +
    ...
    List<Program> findByProgramProfiles_Id(Long id);

    long deleteByNumcontrat(Long numcontrat);
    @Modifying
